...
public class BatchInsertPlugin extends PluginAdapter {
    ...
    public boolean validate(List<String> warnings) {
        ...
        try {
            ...
            if (unFind){
                log.warn("itfsw:插件" + this.getClass().getTypeName() + "插件需配合com.itfsw.mybatis.generator.plugins.ModelColumnPlugin插件使用！");
                return false;
            }
        } catch (Exception e) {
        ...
        // TODO
        log.warn("itfsw:插件" + this.getClass().getTypeName() + "插件如之前使用1.0.5-的插件可继续使用com.itfsw.mybatis.generator.plugins.BatchInsertOldPlugin或者修改batchInsert(@Param(\"list\") List<Tb> list, @Param(\"insertColumns\") Tb.Column ... selective)调用为batchInsertSelective！");
    ...
    public boolean clientGenerated(Interface interfaze, TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        ...
        log.debug("itfsw(批量插入插件):" + interfaze.getType().getShortName() + "增加batchInsertSelective方法。");
    ...
    public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
        ...
        // 使用JDBC的getGenereaterKeys方法获取主键并赋值到keyProperty设置的领域模型属性中。所以只支持MYSQL和SQLServer
        ...
        if (context.getPlugins().sqlMapInsertElementGenerated(element, introspectedTable)) {
            ...
            log.debug("itfsw(批量插入插件):" + introspectedTable.getMyBatis3XmlMapperFileName() + "增加batchInsertSelective实现方法。");
        }
